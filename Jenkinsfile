node {
    ...
    stage("SSH Docker Container Run") {
        def dockerRun = 'sudo docker run -d -p 80:80 --name ${IMAGE_NAME} -v $pwd/letsencrypt:/etc/letsencrypt -v $pwd/nginx:/etc/nginx ${HARBOR_URL}/${HARBOR_PROJECT}/${IMAGE_NAME}:latest'
